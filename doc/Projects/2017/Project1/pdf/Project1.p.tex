...
Use the included function \emph{gauleg} in the file lib.cpp (see also lib.h) to set up the 
weights $\omega_i$ and the points $k_i$. Before you go on 
you need to keep in mind that \emph{gauleg} uses 
...
You will need to map the weights  from \emph{gauleg} to your interval.
To do this, call first \emph{gauleg(a,b,x[],w[],N)}, 
with $a=-1$, $b=1$.
...
\emph{matinv} in the program library lib.cpp (see also lib.h).
